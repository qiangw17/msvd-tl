...
from models.atten_visual_dialog_retrieval import *


def write_results(args,scores, iteration):
        f = open(os.path.join(args.eval_save_path,args.model_name+'_samples_'+str(NUM_NEG_SAMPLES+1)+'_'+args.negative_examples+'_'+str(iteration)+'_result.txt'),'w')
        f.write('############ PARAMETERS ############\n')
        f.write(str(args)+'\n')
        f.write('########### SCORES  ###########\n')
        f.write(str(scores))
...
def write_stat_results(args,scorer, iteration):
    f = open(os.path.join(args.eval_save_path,args.model_name+'_samples_'+str(NUM_NEG_SAMPLES+1)+'_'+args.negative_examples+'_'+str(iteration)+'_stats'),'w')
    f.write('\n'.join(scorer))
    f.close()


NUM_NEG_SAMPLES = 9
def eval_testset(args, sess, model,iteration):
    args_dict = vars(args)
    tmp_args = namedtuple('test_args',args_dict.keys())(**args_dict)
    tmp_args = tmp_args._replace(mode='test')
    tmp_args = tmp_args._replace(batch_size=20)
    if args.word_level:
        vocab = Vocab('/home/jp/data/Game-Based_Video-Context_Dialogue/Twitch-FIFA-Dataset/'+args.dataset+'/vocab_word',max_size=args.vocab_size)
    else:
        vocab = Vocab('/home/jp/data/Game-Based_Video-Context_Dialogue/Twitch-FIFA-Dataset/'+args.dataset+'/vocab',max_size=args.vocab_size)

    data_generator = Batcher(tmp_args,vocab)
    batcher = data_generator.get_batcher()

    print 'calculate scores on the testing set......'
    ...
    while(True):
        ...
        start = time.time()
        # model returns: loss, response_loss, frame_loss, response_probs, frame_probs
        probs = sess.run(model.response_probs,feed_dict={
                                                ...
                                                model.timestamp_offset_batch:batch.get('timestamp_batch')
                                                })
    ...
    write_stat_results(args,save_scorer, iteration)

    # write results
    write_results(args,scores, iteration)
...
def train(args):
    ...
    if args.use_glove:
        if args.word_level:
            wemb = np.load('/home/jp/data/Game-Based_Video-Context_Dialogue/Twitch-FIFA-Dataset/'+args.dataset+'/Wemb_word.npy')
    ...
    while(True):
        ...
        loss, response_loss, frame_loss,debugger,_ = sess.run([model.loss, model.response_loss, model.frame_loss,model.debugger,model.train_op],
                                        feed_dict={
                                            model.video_enc_batch:batch.get('video_batch'),
                                                model.video_enc_mask_batch:batch.get('video_mask_batch'),
                                                model.context_enc_batch:batch.get('chat_context_batch'),
        ...
        iteration +=1
        print 'iteration:',iteration,'computational time:',time.time()-start,'loss:',loss, 'pred_loss:',response_loss

        if iteration > args.max_iter:
            break

        if iteration%args.check_point == 0:
            save_path = os.path.join(args.model_save_path, args.model_name+'-'+str(iteration))
            # print save_path
            # get validation loss and perplexity
            scores = eval_testset(args,sess,model,iteration)
            summary = scores
            summary['ITERATION'] = iteration
            summary_handler.write_summaries(sess,summary)
...
def main():
    ...
    parser.add_argument('--max_video_enc_steps',type=int,default=150,help='number of time steps of the encoder RNN')
    ...
    parser.add_argument('--data_path',type=str,default='/home/jp/data/Game-Based_Video-Context_Dialogue/Twitch-FIFA-Dataset/fifa',help='root location of the dataset')
    parser.add_argument('--dataset',type=str,default='fifa',choices=['twitch','fifa'],help='choose the dataset')
    parser.add_argument('--model_save_path',type=str,default='/home/jp/data/Game-Based_Video-Context_Dialogue/rebuttal/model',help='path where model to be saved')
    parser.add_argument('--summary_save_path',type=str,default='/home/jp/data/Game-Based_Video-Context_Dialogue/rebuttal/summary',help='path where training summary is saved')
    parser.add_argument('--eval_save_path',type=str,default='/home/jp/data/Game-Based_Video-Context_Dialogue/rebuttal/results',help='path where the validation results are stored')
    ...
    parser.add_argument('--video_context',dest='video_context',action='store_true',default=False)
    parser.add_argument('--chat_context',dest='chat_context',action='store_true',default=False)
    ...
    parser.add_argument('--loss_function',type=str,default='3triplet',help='choose from cross-entropy loss or triplet loss')
