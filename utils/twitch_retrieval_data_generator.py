...
class Batcher(object):
    ...
    def _read_data(self):
        ...
        self.h5f = h5py.File(os.path.join('/home/jp/data/Game-Based_Video-Context_Dialogue/handle_raw_videos',self._mode+'_video_feat_fc_50s.h5'),'r')
        



        self.timestamp_data = dict()
        with open(os.path.join('/home/jp/data/Game-Based_Video-Context_Dialogue/handle_raw_videos',self._mode+'_partition_fc_50s.txt'),'r') as f:
            line_data = [line.strip('\n').split(' ') for line in f.readlines()]
            for i in xrange(len(line_data)):
                delta_left = int(line_data[i][1]) - int(line_data[i][3])
                delta_right = int(line_data[i][4]) - int(line_data[i][2])
                self.timestamp_data[line_data[i][0]+'|'+line_data[i][1]+'|'+line_data[i][2]] = [int(line_data[i][1]), int(line_data[i][2]), int(line_data[i][3]), int(line_data[i][4])]
        ...
        for sample in self.json_data:
            ...
            if self._loss_function == '3triplet' and self._mode == 'train':
                self.data.append((video_id,chat_context,responses,labels))
                # print video_id
                # print chat_context
                # print responses
                # print labels
                ...
                for i in range(len(responses)):
                    self.data.append((video_id,chat_context,responses[i],labels[i]))
                    # print video_id, responses[i], labels[i]
    ...
    def _process_data(self):
        ...
        for i in range(0,len(self.data),self._batch_size):
            ...
            original_video_id,original_chat_context,original_response,original_label = zip(*self.data[start:end])
            ...
            original_video_context = [self.h5f[key][:] for key in original_video_id]
            

            response = [chat +' '+STOP_DECODING for chat in original_response] 
            chat_context = original_chat_context
            timestamp = [self.timestamp_data[key][:] for key in original_video_id]
            ...
            chat_context_batch = tf.keras.preprocessing.sequence.pad_sequences(chat_context_ind,padding='post',maxlen=self._max_context_enc_steps)
            response_batch = tf.keras.preprocessing.sequence.pad_sequences(response_ind,padding='post',maxlen=self._max_response_enc_steps)  
            ...
            # video_batch = np.zeros((current_batch_size,self._max_video_enc_steps,8, 8,self._img_dim))
            ...
            timestamp_batch = np.zeros((current_batch_size, self._max_video_enc_steps))
            for idx in xrange(len(original_video_id)):
                timestamp_batch[idx, timestamp[idx][0]-timestamp[idx][2] : timestamp[idx][1]-timestamp[idx][2]] = 1
            ...
            batch.put('timestamp_batch',timestamp_batch)
